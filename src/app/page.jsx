            ...
            <div className="relative">
              ...
              <div
                ...
                onTouchEnd={(e) => {
                  ...
                  if (Math.abs(diff) > 50) {
                    if (diff > 0 && activeTeam < teamMembers.length - 1) setActiveTeam(activeTeam + 1);
                    else if (diff < 0 && activeTeam > 0) setActiveTeam(activeTeam - 1);
                  }
                  setTouchStart(null);
                }}
              >
                {teamMembers.map((member, idx) => (
                  <div key={idx} className="w-full flex-shrink-0 px-4 sm:px-8">
                    <article className="group mx-auto max-w-xs sm:max-w-sm rounded-2xl sm:rounded-3xl border border-white/60 bg-white/80 backdrop-blur-sm p-5 sm:p-6 text-center transition-all duration-300 hover:shadow-xl hover:shadow-teal-100/50">
                      {/* Photo */}
                      <div className="relative mx-auto w-28 h-28 sm:w-36 sm:h-36 mb-4">
                      ...
                      {/* Info */}
                      <h3 className="text-base sm:text-xl font-black text-slate-900">{member.name}</h3>
                      <p className="mt-1 text-xs sm:text-sm font-semibold text-teal-600">{member.role}</p>
                      {member.bio && (
                        <p className="mt-2 text-xs sm:text-sm text-slate-500 leading-5 sm:leading-6">{member.bio}</p>
                      )}
                      ...
                      {(member.telegram || member.instagram || member.linkedin) && (
                        <div className="mt-4 flex items-center justify-center gap-2.5">
                          {member.telegram && (
                            <a href={member.telegram} target="_blank" rel="noopener noreferrer" className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-sky-50 text-sky-500 transition hover:bg-sky-100 hover:text-sky-600 hover:scale-110">
                              <svg className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
                            </a>
                          )}
                          {member.instagram && (
                            <a href={member.instagram} target="_blank" rel="noopener noreferrer" className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-rose-50 text-rose-400 transition hover:bg-rose-100 hover:text-rose-500 hover:scale-110">
                              <svg className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>
                            </a>
                          )}
                          {member.linkedin && (
                            <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-500 transition hover:bg-blue-100 hover:text-blue-600 hover:scale-110">
                              <svg className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                            </a>
                          )}
                        </div>
                      )}
              ...
              {teamMembers.length > 1 && (
                <>
                  <button
                    onClick={() => setActiveTeam(Math.max(0, activeTeam - 1))}
                    disabled={activeTeam === 0}
                    className="absolute left-0 top-1/2 -translate-y-1/2 z-10 flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/90 border border-slate-200 text-slate-600 shadow-lg transition hover:bg-teal-50 hover:text-teal-600 disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg>
                  </button>
                  <button
                    onClick={() => setActiveTeam(Math.min(teamMembers.length - 1, activeTeam + 1))}
                    disabled={activeTeam === teamMembers.length - 1}
                    className="absolute right-0 top-1/2 -translate-y-1/2 z-10 flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/90 border border-slate-200 text-slate-600 shadow-lg transition hover:bg-teal-50 hover:text-teal-600 disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
                  </button>
                </>
              )}
                ...
                <div className="flex justify-center gap-2 mt-6">
                  {teamMembers.map((_, idx) => (
                    <button
                      ...
                      className={`h-2 rounded-full transition-all duration-300 ${idx === activeTeam ? 'w-6 bg-teal-500' : 'w-2 bg-slate-300 hover:bg-slate-400'}`}
                    />
                  ))}
                </div>
              )}
