      ...
      {teamMembers.length > 0 && (
        <section className="relative overflow-hidden py-10 sm:py-16" style={{ background: 'linear-gradient(135deg, #f0fdfa 0%, #ecfeff 40%, #f0f9ff 70%, #f0fdfa 100%)' }}>
          ...
          <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className={`grid gap-5 sm:gap-8 ${teamMembers.length === 1 ? 'max-w-md mx-auto' : teamMembers.length === 2 ? 'grid-cols-1 sm:grid-cols-2 max-w-3xl mx-auto' : teamMembers.length === 3 ? 'grid-cols-1 sm:grid-cols-3 max-w-5xl mx-auto' : 'grid-cols-2 lg:grid-cols-4'}`}>
              {teamMembers.map((member, idx) => (
                <article key={idx} className="group relative rounded-2xl sm:rounded-3xl border border-white/60 bg-white/80 backdrop-blur-sm p-6 sm:p-8 text-center transition-all duration-300 hover:shadow-xl hover:shadow-teal-100/50 hover:-translate-y-1">
                  {/* Photo */}
                  ...
                  {member.bio && (
                    <p className="mt-2.5 text-xs sm:text-sm text-slate-500 leading-5 sm:leading-6">{member.bio}</p>
                  )}
